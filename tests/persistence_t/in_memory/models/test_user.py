...
class UserTest(unittest.TestCase):
    def test_constructor_default_id_is_none(self):
        # when
        user = IMUser('name@example.org', 'hashed_password')
        # then
    ...
    def test_constructor_sets_is_admin(self):
        # when
        user = IMUser('name@example.org', 'hashed_password', False)
        # then
        self.assertFalse(user.is_admin)

        # when
        user = IMUser('name@example.org', 'hashed_password', True)
        # then
    ...
    def test_to_dict_returns_correct_values(self):
        # when
        user = IMUser('name@example.org', '12345')
        # then
